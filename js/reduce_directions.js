...
// is not reasonable, better stay to the same place! So the task is to give 
...
function dirReduc(arr){
  ...
  for (let i = 0; i < arr.length; i++) {
    let opposite = oppositeDir(arr[i]);
    if (i > 0 && opposite === reducedPath[totalTurns - 1]) {
      reducedPath.pop() ;
      totalTurns--;
    } else {
      reducedPath.push(arr[i]);
      totalTurns++;
    }
