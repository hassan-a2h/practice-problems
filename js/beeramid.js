...
// bonus to buy, and build, the largest three-dimensional beer can pyramid 
// you can. And then probably drink those beers, because let's pretend 
// it's Friday too.
...
const beeramid = function(bonus, price) {
  ...
  let levels = 0;
  let cansUsed = 0;
  for (let i = 1; cansUsed < totalCans; i++) {
    if (cansUsed + Math.pow(i, 2) <= totalCans) {
      cansUsed += Math.pow(i, 2);
      levels++; 
    } else {
      break;
    }
