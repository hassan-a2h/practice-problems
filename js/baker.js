...
//   or 200). Ingredients that are not present in the objects, can 
//   be considered as 0.
...
function cakes(recipe, available) {
  ...
  for (let material in recipe) {
    if (!available.hasOwnProperty(material)) {
      return 0;
    }
    
    cakes = Math.min(cakes, Math.floor(available[material] / recipe[material]));
  }
